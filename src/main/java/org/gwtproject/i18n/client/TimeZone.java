...
public class TimeZone implements org.gwtproject.i18n.shared.TimeZone {
  ...
  public static TimeZone createTimeZone(TimeZoneInfo timezoneData) {
    ...
    JsArray<String> jsTimezoneNames = timezoneData.getNames();

    tz.tzNames = new String[jsTimezoneNames.length];

    for (int i = 0; i < jsTimezoneNames.length; i++) {
      tz.tzNames[i] = jsTimezoneNames.getAt(i);
    }

    JsArray<Integer> transitions = timezoneData.getTransitions();
    ...
    if (transitions == null || transitions.length == 0) {
      ...
      for (int i = 0; i < transitionNum; ++i) {
        tz.transitionPoints[i] = transitions.getAt(i * 2);
        tz.adjustments[i] = transitions.getAt(i * 2 + 1);
      }
    }
    return tz;
  }
